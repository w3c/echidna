...
describe('Orchestrator', () => {
  ...
  describe('iterate(iteration, condition, handler, t)', () => {
    ...
    it('should promise an output of same type than input', () => expect(result).to.eventually.be.a('number'));

    it('should properly compute the example value', () => expect(result).to.eventually.equal(42));

    it('should properly iterate over an asynchronous computation', () => {
      function incrementDelay(n) {
        return new Promise((resolve) => {
          setTimeout(() => { resolve(n + 1); }, 25);
        });
      }
  ...
  describe('runStep(step)', () => {
    const dummyRequest = new RequestState().set('jobs', new Map({
      'dummy': new Job()
    }));

    const resultOk = new Orchestrator().runStep(new Map({
      name: 'dummy',
      promise: Promise.resolve(new Map({ status: 'ok' }))
    }))(dummyRequest);

    const resultFailure = new Orchestrator().runStep(new Map({
      name: 'dummy',
    ...
    const resultError = new Orchestrator().runStep(new Map({
      name: 'dummy',
      promise: Promise.resolve(new Map({
        status: 'error',
    ...
    it('should promise a bunch of RequestState objects', () => Promise.all(resultOk.map((promise) => expect(promise).to.be.eventually.an.instanceOf(RequestState)).toArray()));

    it('should set the first returned state as pending job', () => resultOk.get(0).then((state) => expect(state.jobs.get('dummy').status).to.equal('pending')));

    it('should set the second returned state as successful job', () => resultOk.get(1).then((state) => expect(state.jobs.get('dummy').status).to.equal('ok')));

    it('should set the second returned state as successful job', () => resultOk.get(1).then((state) => expect(state.jobs.get('dummy').status).to.equal('ok')));

    it('should set the second returned state as failed job', () => resultFailure.get(1).then((state) => expect(state.jobs.get('dummy').status).to.equal('failure')));

    it('should return a state with errors when a job fails', () => resultFailure.get(1).then((state) => expect(state.jobs.get('dummy').errors).to.have.size(1)));

    it('should return a failed state when a job fails', () => expect(resultFailure.get(1))
        .to.eventually.have.property('status', 'failure'));

    it('should set the second returned state as errored job', () => resultError.get(1).then((state) => expect(state.jobs.get('dummy').status).to.equal('error')));

    it('should return a state with errors when a job errors', () => resultError.get(1).then((state) => expect(state.jobs.get('dummy').errors).to.have.size(1)));

    it('should return an errored state when a job errors', () => expect(resultError.get(1))
