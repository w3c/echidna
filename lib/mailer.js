...
const extractFields = (s, json) => {
  ...
  return {
    ...
    SHORTNAME: M
      ? M.get('thisVersion') && regex.test(M.get('thisVersion'))
        ? M.get('thisVersion').match(regex)[1]
        : '[unknown]'
      : '[unknown]',
    ...
    ENCODEDSOURCE: encodeURIComponent(
      s.url ? `\`${s.url}\`` : s.tar ? `\`${s.tar}\`` : 'a spec',
    ),
...
const sendMessage = (id, state, json, url, ccEmail) => {
  ...
  } else message.text = message.text.replace(new RegExp('@ATTACHMENT'), '');
