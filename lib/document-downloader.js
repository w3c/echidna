...
DocumentDownloader.install = function (dest, content) {
  const path = require('path').dirname(dest);
...
DocumentDownloader.installAll = function (destsContents) {
  ...
  return Promise.all(destsContents.toArray().map((e) => DocumentDownloader.install(e[0], e[1])));
};
...
DocumentDownloader.fetchAndInstall = function (url, dest) {
  ...
  return new Promise((resolve) => {
    Fs.access(dest, Fs.constants.F_OK, (error) => { resolve(!error); });
  }).then((pathExists) => {
    if (!pathExists) return mkdir(dest);
  }).then(() => _.fetch(url).then((content) => {
      ...
      return fileType.fromBuffer(content).then( type => {
        if (type && type.mime !== 'application/x-tar') {
          ...
          return new Promise((resolve, reject) => {
            ...
            extract.on('entry', (header, stream, callback) => {
              stream.on('data', (data) => {
                if (_.isAllowed(header.name)) {
                  ...
                  const path = require('path').dirname(`${dest  }/${  header.name}`);
            ...
            extract.on('finish', () => (hasOverview) ? resolve() :
                reject(
