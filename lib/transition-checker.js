...
TransitionChecker.check = function (profile, latestVersion, previousVersion, isUpdate) {
  return new Promise((resolve, reject) => {
    ...
    if (profile === 'WD' || profile === 'WG-NOTE' || profile === 'IG-NOTE' || profile === 'CRD' || isUpdate) {
      ...
      if (previousVersion.includes('/WD-') || previousVersion.includes('/CR-') || (previousVersion.includes('/CRD-'))) {
        ...
        const shortname = latestVersion.match(new RegExp(/.*\/([^/]+)\/$/))[1];
        const directorApproval = 'transition approved.';
        const commApproval = 'draft transition approved.';
        ...
        .then((content) => {
          for (const issue of content.items) {
              ...
              .then((comments) => {
                ...
                for (const comment of comments.items) {
                  if (comment.body.toLowerCase().startsWith(directorApproval)) {
                    // Director's approval
                    const p1 = octo.teams(global.GH_DIRECTOR_TEAM_ID).members(comment.user.login).fetch((err) => {
                    ...
                    // Comm's approval
                    const p2 = octo.teams(global.GH_COMM_TEAM_ID).members(comment.user.login).fetch((err) => {
        ...
        .catch((error) => reject(new Error(`An error occurred while looking for the transition issue: ${  error}`)));
      }
    }
    else
