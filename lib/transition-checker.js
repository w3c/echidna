      ...
      if (
        ...
        repo.issues
          ...
          .then(content => {
                ...
                repo
                  ...
                  .then(comments => {
                    ...
                    comments.items.every(comment => {
                      if (
                        comment.body.toLowerCase().startsWith(directorApproval)
                      ) {
                        // Director's approval
                        const p1 = octo
                        ...
                        promises.push(p1); // XXX use await?
                      } else if (
                        comment.body.toLowerCase().startsWith(commApproval)
                      ) {
                        // Comm's approval
                        const p2 = octo
                      ...
                      return null; // XXX have to return?
                    });
            ...
            return null; // XXX eslint requires a return for =>
          })
          .catch(error =>
            reject(
              new Error(
