...
function RequestState(status, jobs, history, metadata) {
   ...
   */
  this.addToHistory = fact =>
    this.set('history', this.get('history').add(fact));
  ...
  this.addToMetadata = (key, value) =>
    this.set('metadata', this.get('metadata').set(key, value));
  ...
  this.setJobStatus = (jobName, newStatus) => {
    ...
    return this.set(
      ...
      this.get('jobs').set(
        jobName,
        this.get('jobs').get(jobName).set('status', newStatus),
      ),
    );
  };
