...
function setJobStatus(state, jobName, status) {
  return state.jobs.set(jobName, state.jobs.get(jobName).set('status', status));
}

// Temporary function
function setJobErrors(state, jobName, errors) {
  return state.jobs.set(jobName, state.jobs.get(jobName).set('errors', errors));
}
...
Orchestrator.prototype.runStep = function (step) {
  return function (state) {
    state.jobs = setJobStatus(state, step.get('name'), 'pending');
    return List.of(
      Promise.resolve(state),
      step.get('promise').then(function (obj) {
        state.jobs = setJobStatus(state, step.get('name'), obj.get('status'));

        if (obj.has('errors')) {
          state.jobs = setJobErrors(state, step.get('name'), obj.get('errors'));
        }
        if (obj.has('history')) {
...
Orchestrator.prototype.next = function (state) {
  if (state.jobs.get('retrieve-resources').get('status') === '') {
    ...
    state.history = state.history.add(
      ...
      '<a href="' + state.metadata.get('thisVersion') + '">' +
