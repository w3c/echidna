...
function trInstaller(source, dest) {
  return new Promise((resolve, reject) => {
    const cmd = global.TR_INSTALL_CMD
...
const Orchestrator = function (url,
                             tar,
                             token,
                             user,
                             tempLoc,
...
Orchestrator.iterate = function (iteration, condition, handler, t) {
  ...
  return iteration(t).reduce((sequence, promisedNewT) => sequence.then(() => promisedNewT.then((newT) => {
        handler(newT);
        return newT;
      })), Promise.resolve()).then((finalT) => Orchestrator.iterate(iteration, condition, handler, finalT));
};
  ...
  if (isTar) {
    ...
    map = new Map({
      ...
      promise: DocumentDownloader.install(tempLocation + tar.originalname,
                                          tar.buffer)
        .then(() => DocumentDownloader.fetchAndInstall(finalUrl, tempLocation)).then(() => {
...
Orchestrator.prototype.runIPChecker = function (ip) {
  return new Map({
    name: 'ip-checker',
...
Orchestrator.prototype.runSpecberus = function (httpLocation,
  ...
  return new Map({
    name: 'specberus',
    promise: SpecberusWrapper.validate(httpLocation, profile, recTrack, patentPolicy)
      .then((report) => {
...
Orchestrator.prototype.runTransitionChecker = function (profile, latestVersion, previousVersion, isUpdate) {
  return new Map({
    name: 'transition-checker',
    promise: TransitionChecker.check(profile, latestVersion, previousVersion, isUpdate)
      .then((report) => {
        if (report.errors.isEmpty()) {
          return new Map({
            status: 'ok',
...
Orchestrator.prototype.runTokenChecker = function (latestVersion, token) {
  return new Map({
    name: 'token-checker',
...
Orchestrator.prototype.runTrInstaller = function (thisVersion, tempLocation) {
  ...
  return new Map({
    name: 'tr-install',
    promise: trInstaller(tempLocation, finalTRpath).then(() => new Map({ status: 'ok' })).catch((error) => new Map({
        status: 'error',
        errors: List.of(error.toString()),
...
Orchestrator.prototype.runShortlink = function (thisVersion) {
  return new Map({
    ...
    promise: updateTrShortlink(thisVersion).then(() => new Map({ status: 'ok' })).catch((error) => new Map({
        status: 'error',
        errors: List.of(error.toString()),
