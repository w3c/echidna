...
import DocumentDownloader from './document-downloader.js';
import JsonHttpService from './json-http-service.js';
...
import TransitionChecker from './transition-checker.js';
import IPChecker from './ip-checker.js';
...
Orchestrator.prototype = {
  ...
  next(originState) {
    ...
    if (state.hasJobStarted('ip-checker')) {
      ...
      step = this.runSpecberus(this.httpLocation, state.get('metadata'));
    ...
    const thisVersion = state.get('metadata').get('thisVersion');
    const link = `<a href="${thisVersion}">${thisVersion}</a>`;
    state = state.addToHistory(
      this.dryRun
        ? `Dry run: The document would be published at ${link} without the dry-run option.`
        : `The document has been published at ${link}.`,
    );
      ...
      .then(metadata => {
        if (metadata) {
          const m = {
            ...
            recCandidateAmendments:
              metadata.cSubChanges || metadata.cNewFeatures,
            recProposedAmendments:
              metadata.pSubChanges || metadata.pNewFeatures,
          };
          if (metadata.profile === 'REC') {
            m.recCandidateAmendments =
              metadata.cSubChanges || metadata.cNewFeatures;
            m.recProposedAmendments =
              metadata.pSubChanges || metadata.pNewFeatures;
            m.errata = metadata.errata;
          }
          return new Map({
            status: 'ok',
...
Orchestrator.prototype.runSpecberus = (httpLocation, metadata) =>
  new Map({
    name: 'specberus',
    promise: SpecberusWrapper.validate(httpLocation, metadata)
      .then(report => {
