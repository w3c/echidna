...
Orchestrator.prototype.runStep = function (step) {
  return function (state) {
    return step.then(function (obj) {
      state.jobs[obj.name].status = obj.status;

      if (obj.errors) state.jobs[obj.name].errors = obj.errors;
      if (obj.history) state.history = state.history.add(obj.history);
      if (obj.metadata) state.metadata = obj.metadata;
