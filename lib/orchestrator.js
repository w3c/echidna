...
/**
 * Iteratively calls a function over a promised value until a condition is reached
 *
 * @param {T => List.<T, Promise.<T>>} iteration - A function to repeatedly generate a new T from a previous T until a condition is reached
 * @param {T => Boolean} condition - A function that returns true when the iteration must stop for a given T
 * @param {T => ()} handler - A function to handle both values produced by `iteration`, the second one after the promise is fulfilled
 * @param {T} seed - A starting value
 * @returns {Promise.<T>} The result of the last call to `iteration` before it stops
 */
Orchestrator.iterate = function (iteration, condition, handler, seed) {
  if (condition(seed)) return Promise.resolve(seed);
  else {
    var newSeeds = iteration(seed);
    handler(newSeeds.get(0));
    return newSeeds.get(1).then(function (newSeed) {
      handler(newSeed);
      return Orchestrator.iterate(iteration, condition, handler, newSeed);
    });
  }
};
 ...
 *
 * @param {Object} state - A state for a specific publication request
 * @returns {List.<Object, Promise.<Object>>} A pair containing (1) the new state when the next step starts and (2) a promise of a state when the next step ends
 */
