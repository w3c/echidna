...
// Temporary function
function setJobStatus(state, jobName, status) {
  return state.set(
    'jobs',
    state.get('jobs').set(
      jobName,
      state.get('jobs').get(jobName).set('status', status)
    )
  );
}

// Temporary function
function setJobErrors(state, jobName, errors) {
  return state.set(
    'jobs',
    state.get('jobs').set(
      jobName,
      state.get('jobs').get(jobName).set('errors', errors)
    )
  );
}
...
Orchestrator.prototype.runStep = function (step) {
  return function (state) {
    return List.of(
      Promise.resolve(setJobStatus(state, step.get('name'), 'pending')),
      step.get('promise').then(function (obj) {
        state = setJobStatus(state, step.get('name'), obj.get('status'));

        if (obj.has('errors')) {
          state = setJobErrors(state, step.get('name'), obj.get('errors'));
        }
        if (obj.has('history')) {
          state = state.set('history', state.get('history')
                            .add(obj.get('history')));
        }
        if (obj.has('metadata')) {
      ...
      .catch(function (err) {
        ...
        state = state.set('history', state.get('history').add(
...
Orchestrator.prototype.next = function (state) {
  ...
  else if (state.get('jobs').get('update-tr-shortlink').get('status') === '') {
    return this.runStep(
