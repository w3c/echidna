...
Orchestrator.prototype.runSpecberus = function (httpLocation,
  ...
  return new Map({
    name: 'specberus',
    promise: SpecberusWrapper.validate(httpLocation, profile, recTrack)
...
/**
 * Run the tinkering step.
 * @param {String} tempLocation temporary, local path to the document.
 */

Orchestrator.prototype.runTinkerer = function (tempLocation) {
  Tinkerer.tinker(tempLocation);
};
...
Orchestrator.prototype.next = function (state) {
  ...
  else if (state.hasJobStarted('tinker')) {
    step = this.runTinkerer(this.tempLocation);
  }
