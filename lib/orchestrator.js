...
var Orchestrator = function (url, tar, token, user,tempLoc, httpLoc, resultLoc) {
  this.url = url;
  this.tar = tar;
  this.token = token;
  this.user = user;
      ...
      .then(function (metadata) {
        if (metadata.profile && metadata.delivererIDs) {
          ...
          return new Map({
            ...
            history: 'Couldn\'t extract the metadata.'
          });
        }
    }).catch(function (error) {
      return new Map({
...
Orchestrator.prototype.runSpecberus = function (httpLocation, profile) {
  return new Map({
    name: 'specberus',
    promise: SpecberusWrapper.validate(httpLocation, profile)
...
Orchestrator.prototype.next = function (state) {
  ...
  if (state.hasJobStarted('retrieve-resources')) {
    ...
    step = this.runUserChecker(this.user, state.get('metadata').get('delivererIDs'));
  }
  else if (state.hasJobStarted('specberus')) {
    step = this.runSpecberus(this.httpLocation, state.get('metadata').get('profile'));
  }
